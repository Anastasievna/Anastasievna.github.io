...
async function createBean(bean:beanType) {
  ...
  const fileName = `${newBean.details.region}-${newBean.details.process}`.toLowerCase();
  const pathName = path.join("data", "beans", `${fileName}.json`);
  const response = await createFile(JSON.stringify(newBean, null, 2), pathName);
  if (response.type === dataTypes.SUCCESS) return createData(dataTypes.SUCCESS, {id: newBean.id})
...
async function updateBean(id:string, bean: beanType) {
  ...
  try {
    ...
    if (!!targetBean && targetBean.path) {
      ...
      const response = await createFile(JSON.stringify(newBean, null, 2), targetBean.path, true)
      if (response.type === dataTypes.SUCCESS) return createData(dataTypes.SUCCESS, "File updated successfull")
      else return response; 
    } else {
...
async function deleteBean(id:string) {
  ...
  try {
    ...
    if (!!targetBean && targetBean.path) {
      ...
      if (response.type === dataTypes.SUCCESS) return createData(dataTypes.SUCCESS, "File deleted successfull")
      else return response;
    } else {
...
function checkBeansData(bean: beanType) {
  ...
  return createData(
      dataTypes.ERROR, "Fields required is empty"
    );
}

function setDefaultPropsBean(bean: beanType): beanType {
  bean.id = uuidv4(); 
  if (bean.recipes === undefined || !bean.recipes?.length) bean.recipes = DefaultRecipes;

  return bean;
}


export const beansService = { getBeans, parseBeans, checkBeansData, createBean, updateBean, deleteBean};
